...
class Node(KV_store):
	...
	def add_shard(self, nodes):
		...
		for v_shard in range(self.virtual_range):
			...
			if self.virtual_translation[predecessor] == self.shard_ID:
				self.atomic_key_transfer(predecessor, ring_num, successor, 'update') 
	...
	def remove_shard(self, shard_ID):
		print('removing shard, should do a final state-transfer', file=sys.stderr)
		if shard_ID == self.shard_ID:

			# remove all virtual shards for this physical shard
			for v_shard in self.V_SHARDS:
				if self.virtual_translation[v_shard] == shard_ID:
					self.V_SHARDS.pop(v_shard)

			

			# note: all replicas in this shard will be doing this concurrently
			# will that be a problem

			self.atomic_key_transfer(0, 0, 0, 'delete')
	...
	'''
	def atomic_key_transfer(self, predecessor, new_shard, successor, Type):
		
		for key in list(self.keystore):
			key_hash = self.hash(key)
			
			if Type == 'update' and key_hash < successor and key_hash > predecessor:
				shard_destination = self.virtual_translation[new_shard]
				send_key = True

			if Type == 'delete':
				shard_destination = self.find_match(key)
				send_key = True
